...
extension UICollectionView {
    func scrollItemToScreenCenter(at indexPath: IndexPath, cellWidth: CGFloat) {
        guard let attributes = layoutAttributesForItem(at: indexPath) else { return }
        let cellCenterX = attributes.frame.midX
        let collectionViewInScreen = convert(bounds, to: nil)
        let screenCenterX = UIScreen.main.bounds.width / 2
        let offsetX = cellCenterX - (screenCenterX - collectionViewInScreen.origin.x)
        let newOffsetX = max(0, min(contentSize.width - bounds.width, offsetX))
        setContentOffset(CGPoint(x: newOffsetX, y: 0), animated: true)
    }
}
