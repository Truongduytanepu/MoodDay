...
class PermissionHelper {
    ...
    var hasCameraAndMicroPermission: Bool {
        return self.hasCameraPermission && self.hasMicrophonePermission
    }

    func requestWritingPhotoPermission(completion: @escaping RequestPermissionCompletion) {
        if #available(iOS 14, *) {
            let previousStatus = PHPhotoLibrary.authorizationStatus(for: .readWrite)
            PHPhotoLibrary.requestAuthorization(for: .readWrite) { status in
                DispatchQueue.main.async {
                    completion(status == .authorized, previousStatus == .denied || status == .limited)
                }
            }
        } else {
    ...
    func requestAccessPhotoPermission(completion: @escaping RequestPermissionCompletion) {
        if #available(iOS 14, *) {
            let previousStatus = PHPhotoLibrary.authorizationStatus(for: .readWrite)
            PHPhotoLibrary.requestAuthorization(for: .readWrite) { status in
                DispatchQueue.main.async {
                    completion(status == .authorized || status == .limited, previousStatus == .denied)
                }
            }
        } else {
            let previousStatus = PHPhotoLibrary.authorizationStatus()
